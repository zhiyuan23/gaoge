...
- 备份目录权限为 `700`。
- 回滚状态先明确记录旧环境存在或不存在；存在时通过临时文件复制、内容比对和原子重命名保存一份权限为 `600` 的旧环境文件，状态目录权限为 `700`。只有环境安装已经开始时才允许恢复旧环境；恢复前先在 `shared` 同目录预暂存并校验，切换 release 后仅原子激活，激活失败时补偿切回原 release。发布成功或回滚步骤退出时立即删除状态，后续发布还会清理超过 7 天的异常残留目录。
- 守卫失败信息只包含脱敏数据库身份和计数。

...
3. 通过环境变量向 PostgreSQL CLI 传递连接信息，避免密码出现在命令参数和日志中。
4. 调用 `psql` 查询：
   - `inet_server_addr()`
   - `inet_server_port()`
   - `current_database()`
   - 用户、球员、球队、比赛和资产记录数量
5. 要求实际地址为 `::1`、端口为 `5432`、库名为 `gaoge_db`。
6. 要求五类关键业务表的数量均大于零。
7. 输出只包含脱敏后的地址、端口、库名和计数，不输出用户名、密码或完整连接串。
...
回滚状态放在 `${API_DEPLOY_PATH}/tmp`，文件名包含 Git SHA，避免不同发布相互覆盖。

...
1. Prisma 新增 `Team`、`PlayerTeam`、`MatchRound`、`MatchRoundTeamResult`，并给 `Player` 增加 `jerseyName`
2. 初始化固定 3 支球队
3. 迁移旧 `subTeam` 数据到 `PlayerTeam`
4. 调整 `apps/api` 的 DTO、查询、返回结构
5. 调整 `apps/admin` 的球员页面录入与筛选
6. 新增比赛轮次录入与积分榜接口、页面
7. 移除旧 `subTeam` 字段及其前后端兼容逻辑
...
这样既满足当前“3 支固定球队 + 球员兼属 + 单轮积分榜 + 总积分榜”的明确需求，也不会提前引入超出当前范围的复杂抽象。

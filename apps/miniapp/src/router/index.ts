import rawPagesJson from '@/pages.json'
 ...
 */
const parseRoutes = (pagesJson: any = {}) => {
  if (!pagesJson.pages) {
    pagesJson.pages = []
  }
...
export const isTabBarPath = (path = '') => {
  const cleanPath = removeQueryString(path)
  return pagesJson.tabBar?.list?.some((item) => `/${item.pagePath}` === cleanPath) === true
}

  ...
  if (val) {
    nextTick(() => {
      if (dialogContentRef.value) {
        const modalContent = dialogContentRef.value as { el?: { $el?: HTMLElement } } | null
        dialogRef.value = modalContent?.el?.$el
        setTransform()
      }
    })
  }
...
<template>
  <Dialog :modal="props.overlay" :open="isOpen" @update:open="updateOpen">
    <DialogContent
      ...
      :class="
        cn(
          'md:min-h-auto left-0 right-0 top-0 mx-auto flex h-[calc-size(auto,size)] max-h-full min-h-full translate-x-0 translate-y-0 flex-col gap-0 p-0 md:top-[5vh] md:max-h-[90vh]',
          props.class,
          {
            'size-full max-h-full max-w-full md:top-0 md:max-h-full': isMaximize,
            'md:-translate-y-1/2! md:top-1/2': props.alignCenter,
    ...
    >
      ...
      >
        ...
        <slot name="header">
          <DialogTitle
            class="flex-center gap-x-2 md:justify-start"
            :class="{ 'md:justify-center': props.center }"
          >
            <FaIcon
              v-if="props.icon"
              :name="

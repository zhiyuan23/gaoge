...
<template>
  <div>
    ...
    <FaPageMain v-for="(item, index) in rules" :key="index" :title="item.title">
      ...
      <div class="space-x-2">
        <FaButton @click="copy(item.rule.toString())"> 复制 </FaButton>
        <FaButton variant="outline" @click="test(index)"> 测试 </FaButton>
      </div>
    </FaPageMain>
    <ElDialog v-model="dialog.visible" :title="rules[dialog.index].title" width="500px">
      <ElForm :model="dialog.form" :rules="dialog.formRules">
        <ElFormItem prop="test">
          <ElInput
            v-model="dialog.form.test"
            :placeholder="`例如：${rules[dialog.index].examples.join('、')}`"
          />
        </ElFormItem>
      </ElForm>
    </ElDialog>

...
<template>
  <div>
    <FaPageHeader
      title="VueUse"
      description="VueUse 是一个 Vue Composition API 实用程序的集合，更多 API 和例子请查看 VueUse 官网。"
    >
    ...
    <FaPageMain title="防抖：debounce">
      ...
      <ElButton @click="debounce"> 连续点击我，只会执行最后一次点击事件 </ElButton>
    </FaPageMain>
    <FaPageMain title="节流：throttle">
      <p class="mt-0">
        所谓节流，就是指连续触发事件但是在 n 秒中只执行一次函数。节流会稀释函数的执行频率。
      </p>
      <ElButton @click="throttle"> 连续点击我，每一秒只会执行一次点击事件 </ElButton>
    </FaPageMain>
    <FaPageMain v-if="isSupported" title="复制：clipboard">
      <ElInput v-model="input" placeholder="输入内容，并点击复制按钮">
        <template #append>
          <ElButton @click="copy(input)"> 复制 </ElButton>
        </template>
      </ElInput>
    </FaPageMain>
  </div>

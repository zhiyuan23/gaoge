...
<template>
  <div>
      ...
      <div class="space-y-2">
        <div class="space-x-2">
          <FaButton
            ...
            :key="index"
            :variant="userStore.account === item.value ? 'default' : 'outline'"
            @click="accountChange(item.value)"
          >
            {{ item.label }}
          </FaButton>
        </div>
    ...
    <FaPageMain v-if="settingsStore.settings.app.enablePermission" title="路由鉴权">
      <FaButton @click="goTest"> 跳转页面 </FaButton>
    </FaPageMain>
    ...
    <FaPageMain v-if="settingsStore.settings.app.enablePermission" title="鉴权函数">
      <div class="space-y-2">
        ...
        <div class="space-x-2">
          ...
          <FaButton
            variant="outline"
            @click="permissionCheck2(['permission.browse', 'permission.create'])"
          >

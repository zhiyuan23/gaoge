...
const { setValue, setOptions, numberValue, formattedValue } = useCurrencyInput(
  options.value as CurrencyInputOptions,
)
...
<template>
  <div>
    ...
    <FaPageMain>
      <div class="md-grid-cols-2 md-gap-x-8 grid items-center gap-y-4">
    ...
    <FaPageMain title="选项">
      <template #title>
        <div class="flex items-center justify-between">
          选项
          <ElButton v-if="isSupported" size="small" @click="handleCopy"> 复制选项 </ElButton>
        </div>
      </template>
      <div class="md-grid-cols-2 grid grid-cols-1 gap-x-8">
        <div>
          ...
          <Option.reuse label="货币显示" description="如何在格式化中显示货币">
            <ElSelect v-model="currencyDisplay">
              <ElOption
                v-for="item in currencyDisplays"
                :key="item.value"
                :value="item.value"
                :label="item.label"
              />
        ...
        <div>
          <Option.reuse
            label="数值范围"
            description="验证在模糊时触发，并在超出范围时自动设置相应的阈值"
          >
            <template #switch>
          ...
          <Option.reuse label="数值缩放" description="对输出值进行缩放">
            ...
            <ElSelect v-model="valueScaling" :disabled="!valueScalingEnabled">
              <ElOption
                v-for="item in valueScalingOptions"
                :key="item.value"
                :value="item.value"
                :label="item.label"
              />
          ...
          <Option.reuse
            label="自动小数位"
            description="是否自动插入小数符号，将最后输入的数字作为小数位"
          >
            <template #switch>

        ...
        if (index !== -1) {
          tabRef.value &&
            tabsRef.value?.scrollTo(
              tabRef.value[index].offsetLeft - tabsRef.value.ref?.$el.clientWidth * 0.4,
            )
          tabbarScrollTip()
        }
      })
...
function tabbarScrollTip() {
  if (
    tabContainerRef.value?.$el.clientWidth > (tabsRef.value?.ref?.$el.clientWidth ?? 0) &&
  ...
  <div class="tabbar">
    ...
    <div class="tabbar-container">
      <FaScrollArea
        ...
        :scrollbar="false"
        mask
        gradient-color="var(--g-tabbar-bg)"
        class="tabs"
      >
        ...
        .tab {
          ...
          * {
          ...
          .tab-content {
            ...
            .action-icon {
              --uno: transition absolute inset-e-2 top-1/2 -translate-y-1/2 rounded-full z-10 w-5
                h-5 flex-center text-xs 'text-[var(--g-tabbar-tab-color)]'
                hover: (border bg-secondary);
            }

            .hotkey-number {
              --uno: border bg-secondary absolute inset-e-2 top-1/2 -translate-y-1/2 rounded-full
                z-10 w-5 h-5 flex-center text-xs 'text-[var(--g-tabbar-tab-color)]';
            }
          }
        }
      }

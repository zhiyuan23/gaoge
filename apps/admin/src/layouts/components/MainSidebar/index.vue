...
<template>
  <Transition name="main-sidebar">
    <div
      v-if="
        settingsStore.settings.menu.mode === 'side' ||
        (settingsStore.mode === 'mobile' && settingsStore.settings.menu.mode !== 'single')
      "
    ...
    >
      ...
      <FaScrollArea :scrollbar="false" mask gradient-color="var(--g-main-sidebar-bg)" class="menu">
        <!-- 侧边栏模式（含主导航） -->
        <div class="of-hidden -mt-2 flex w-full flex-col py-1 transition-all">
          <template v-for="(item, index) in menuStore.allMenus" :key="index">
            ...
            >
              <div
                ...
                class="menu-item-container hover-(bg-[var(--g-main-sidebar-menu-hover-bg)] text-[var(--g-main-sidebar-menu-hover-color)]) px-2! group relative flex h-full w-full cursor-pointer items-center justify-between gap-1 rounded-lg py-4 text-[var(--g-main-sidebar-menu-color)] transition-all"
                :class="{
              ...
              >
                <div
                  class="inline-flex w-full flex-1 flex-col items-center justify-center gap-[2px]"
                >
                  <FaIcon
                    v-if="item.meta?.icon"
                    :name="item.meta?.icon"
                    class="menu-item-container-icon group-hover-scale-120 transition-transform"
                  />
                  <span

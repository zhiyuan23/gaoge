...
describe('AssetRecordService', () => {
  ...
  it('returns paged list with keyword and season filters', async () => {
    ...
    prisma.footballAssetRecord.findMany.mockResolvedValue([{ id: 2 }])
    prisma.footballAssetRecord.count.mockResolvedValue(4)
    ...
    expect(prisma.footballAssetRecord.findMany).toHaveBeenCalledWith({
      where: {
        OR: [
          {
            title: {
  ...
  it('rejects waived records unless they are zero-amount match-fee income', async () => {
    ...
    expect(prisma.footballAssetRecord.create).not.toHaveBeenCalled()
  })

  it('creates records with normalized optional text and creator id', async () => {
    const { prisma, service } = createService()
    prisma.footballAssetRecord.create.mockResolvedValue({ id: 8 })
    ...
    expect(prisma.footballAssetRecord.create).toHaveBeenCalledWith({
      data: {
        direction: 'income',
        recordType: 'match_fee',
        amount: 2000,
  ...
  it('returns confirmed summary totals only', async () => {
    const { prisma, service } = createService()
    prisma.footballAssetRecord.aggregate
      .mockResolvedValueOnce({ _sum: { amount: 120000 } })
      .mockResolvedValueOnce({ _sum: { amount: 66800 } })
    prisma.footballAssetRecord.count.mockResolvedValue(1)
  ...
  it('throws when the asset record does not exist', async () => {
    const { prisma, service } = createService()
    prisma.footballAssetRecord.findUnique.mockResolvedValue(null)

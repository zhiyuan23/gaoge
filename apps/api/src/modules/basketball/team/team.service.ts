...
export class TeamService {
  ...
  create(dto: CreateTeamDto) {
    return this.prisma.basketballTeam.create({
  ...
  async findAll(params: TeamListParams = {}) {
    ...
    const [list, total] = await this.prisma.$transaction([
      this.prisma.basketballTeam.findMany({
        where,
        skip: (page - 1) * pageSize,
        take: pageSize,
        orderBy: [{ sort: 'asc' }, { createdAt: 'asc' }],
      }),
      this.prisma.basketballTeam.count({ where }),
    ])
  ...
  async findOne(id: number) {
    const team = await this.prisma.basketballTeam.findUnique({ where: { id } })
  ...
  async update(id: number, dto: UpdateTeamDto) {
    ...
    return this.prisma.basketballTeam.update({
      where: { id },
  ...
  async remove(id: number) {
    await this.findOne(id)
    return this.prisma.basketballTeam.delete({ where: { id } })
  }
}
...
function buildTeamWhere(params: TeamListParams) {
  ...
  const where: Prisma.BasketballTeamWhereInput = {}

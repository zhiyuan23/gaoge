...
describe('TeamService', () => {
  ...
  it('returns paged list and total with keyword filtering and stable ordering', async () => {
    ...
    prisma.basketballTeam.findMany.mockResolvedValue([{ id: 2 }, { id: 3 }])
    prisma.basketballTeam.count.mockResolvedValue(12)
    ...
    expect(prisma.basketballTeam.findMany).toHaveBeenCalledWith({
      where: {
        name: {
          contains: '皇家',
          mode: 'insensitive',
    ...
    expect(prisma.basketballTeam.count).toHaveBeenCalledWith({
      where: {
        name: {
          contains: '皇家',
          mode: 'insensitive',
  ...
  it('creates a team with the mapped fixed team code', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.create.mockResolvedValue({ id: 1 })
    ...
    expect(prisma.basketballTeam.create).toHaveBeenCalledWith({
  ...
  it('rejects create when the team name is not one of the fixed teams', async () => {
    ...
    expect(prisma.basketballTeam.create).not.toHaveBeenCalled()
  })
  ...
  it('throws when the team does not exist', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.findUnique.mockResolvedValue(null)
  ...
  it('returns the team when it exists', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.findUnique.mockResolvedValue({ id: 3, name: '皇家高歌' })
  ...
  it('checks existence before update', async () => {
    ...
    expect(prisma.basketballTeam.update).not.toHaveBeenCalled()
  })

  it('updates the team after existence check and normalizes blank optional text fields', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.findUnique.mockResolvedValue({ id: 7, name: '旧队名' })
    prisma.basketballTeam.update.mockResolvedValue({ id: 7, slogan: null, sponsorName: null })
    ...
    expect(prisma.basketballTeam.update).toHaveBeenCalledWith({
      where: { id: 7 },
      data: {
        name: '高歌联',
        code: 'united',
  ...
  it('keeps undefined nullable fields untouched on update', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.findUnique.mockResolvedValue({
      id: 8,
      name: '旧队名',
      slogan: '保留口号',
    })
    prisma.basketballTeam.update.mockResolvedValue({ id: 8, name: '新队名', slogan: '保留口号' })
  ...
  it('treats explicit null as clearing nullable fields on update', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.findUnique.mockResolvedValue({ id: 10, name: '旧队名' })
    prisma.basketballTeam.update.mockResolvedValue({ id: 10, slogan: null, sponsorName: null })
  ...
  it('checks existence before remove', async () => {
    ...
    expect(prisma.basketballTeam.delete).not.toHaveBeenCalled()
  })

  it('removes the team after existence check', async () => {
    const { prisma, service } = createService()
    prisma.basketballTeam.findUnique.mockResolvedValue({ id: 9, name: '皇家高歌' })
    prisma.basketballTeam.delete.mockResolvedValue({ id: 9 })

    await expect(service.remove(9)).resolves.toEqual({ id: 9 })

    expect(prisma.basketballTeam.delete).toHaveBeenCalledWith({ where: { id: 9 } })
  })
})

...
export class AssetRecordService {
  ...
  create(dto: CreateAssetRecordDto, creatorId: number) {
    ...
    return this.prisma.basketballAssetRecord.create({
      data,
    })
  }
  ...
  async findAll(params: AssetRecordListDto | AssetRecordListParams = {}) {
    ...
    const [list, total] = await this.prisma.$transaction([
      this.prisma.basketballAssetRecord.findMany({
        where,
        skip: (page - 1) * pageSize,
        take: pageSize,
        orderBy: [{ recordDate: 'desc' }, { createdAt: 'desc' }],
      }),
      this.prisma.basketballAssetRecord.count({ where }),
    ])
  ...
  async getSummary(): Promise<AssetRecordSummary> {
    const [income, expense, waivedMatchCount] = await Promise.all([
      this.prisma.basketballAssetRecord.aggregate({
        where: {
          direction: 'income',
          status: 'confirmed',
        },
      ...
      this.prisma.basketballAssetRecord.count({
        where: {
          direction: 'income',
  ...
  async findOne(id: number) {
    const record = await this.prisma.basketballAssetRecord.findUnique({
      where: { id },
    })
  ...
  async update(id: number, dto: UpdateAssetRecordDto) {
    ...
    return this.prisma.basketballAssetRecord.update({
      where: { id },
  ...
  async remove(id: number) {
    ...
    return this.prisma.basketballAssetRecord.delete({
      where: { id },
    })
  }
}
...
function buildAssetRecordWhere(params: AssetRecordListDto | AssetRecordListParams) {
  ...
  const where: Prisma.BasketballAssetRecordWhereInput = {}

...
export class MatchRoundService {
  ...
  async create(dto: CreateMatchRoundDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const round = await tx.basketballMatchRound.create({ data })

      await tx.basketballMatchRoundResult.createMany({
      ...
      const created = await tx.basketballMatchRound.findUnique({
        where: { id: round.id },
  ...
  async findAll(params: MatchRoundListParams = {}) {
    ...
    const [list, total] = await this.prisma.$transaction([
      this.prisma.basketballMatchRound.findMany({
        where,
      ...
      this.prisma.basketballMatchRound.count({ where }),
    ])
  ...
  async findOne(id: number) {
    const round = await this.prisma.basketballMatchRound.findUnique({
      where: { id },
      include: MATCH_ROUND_INCLUDE,
    })
  ...
  async update(id: number, dto: UpdateMatchRoundDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.basketballMatchRound.update({
        where: { id },
        data,
      })

      if (results) {
        await tx.basketballMatchRoundResult.deleteMany({
          where: {
            matchRoundId: id,
          },
        })
      ...
      const updated = await tx.basketballMatchRound.findUnique({
        where: { id },
        include: MATCH_ROUND_INCLUDE,
      })
  ...
  async remove(id: number) {
    await this.findOne(id)
    return this.prisma.basketballMatchRound.delete({
      where: { id },
    })
  }
}
...
function buildMatchRoundWhere(params: MatchRoundListParams) {
  ...
  const where: Prisma.BasketballMatchRoundWhereInput = {}
...
async function validateTeamResults(
  ...
  const total = await tx.basketballTeam.count({
    where: {
      id: {
        in: teamIds,
      },

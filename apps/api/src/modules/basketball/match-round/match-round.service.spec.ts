...
describe('MatchRoundService', () => {
  ...
  it('creates a match round in a transaction and derives points from rank', async () => {
    ...
    tx.basketballMatchRound.create.mockResolvedValue({ id: 8 })
    tx.basketballMatchRound.findUnique.mockResolvedValue({
      ...roundRecord,
      id: 8,
    })
    ...
    expect(tx.basketballTeam.count).toHaveBeenCalledWith({
      where: {
        id: {
          in: [11, 12, 13],
        },
    ...
    expect(tx.basketballMatchRound.create).toHaveBeenCalledWith({
      data: {
        year: 2026,
        season: '春季赛',
        round: 2,
    ...
    expect(tx.basketballMatchRoundResult.createMany).toHaveBeenCalledWith({
      data: [
  ...
  it('rejects results when some teams do not exist', async () => {
    ...
    tx.basketballTeam.count.mockResolvedValue(2)
    ...
    expect(tx.basketballMatchRound.create).not.toHaveBeenCalled()
    expect(tx.basketballMatchRoundResult.createMany).not.toHaveBeenCalled()
  })
  ...
  it('returns paged list with filters, team names and match ordering', async () => {
    ...
    prisma.basketballMatchRound.findMany.mockResolvedValue([roundRecord])
    prisma.basketballMatchRound.count.mockResolvedValue(5)
    ...
    expect(prisma.basketballMatchRound.findMany).toHaveBeenCalledWith({
      where: {
        year: 2026,
        season: '春季赛',
        round: 1,
    ...
    expect(prisma.basketballMatchRound.count).toHaveBeenCalledWith({
      where: {
        year: 2026,
        season: '春季赛',
        round: 1,
  ...
  it('returns detail with team names when the round exists', async () => {
    const { prisma, service } = createService()
    prisma.basketballMatchRound.findUnique.mockResolvedValue(roundRecord)
  ...
  it('rejects detail serialization when stored results violate the three-team invariant', async () => {
    const { prisma, service } = createService()
    prisma.basketballMatchRound.findUnique.mockResolvedValue({
      ...roundRecord,
  ...
  it('throws a not found exception when the round does not exist', async () => {
    const { prisma, service } = createService()
    prisma.basketballMatchRound.findUnique.mockResolvedValue(null)
  ...
  it('updates the round by rebuilding results inside a transaction', async () => {
    const { prisma, service, tx } = createService()
    prisma.basketballMatchRound.findUnique.mockResolvedValue(roundRecord)
    tx.basketballMatchRound.update.mockResolvedValue({ id: 1 })
    tx.basketballMatchRound.findUnique.mockResolvedValue({
      ...roundRecord,
      venue: null,
      remark: null,
    ...
    expect(tx.basketballMatchRound.update).toHaveBeenCalledWith({
      where: { id: 1 },
      data: {
        year: 2026,
        season: '夏季赛',
    ...
    expect(tx.basketballMatchRoundResult.deleteMany).toHaveBeenCalledWith({
      where: {
        matchRoundId: 1,
      },
    })
  ...
  it('updates only the main round when results are omitted', async () => {
    ...
    expect(tx.basketballMatchRoundResult.deleteMany).not.toHaveBeenCalled()
    expect(tx.basketballMatchRoundResult.createMany).not.toHaveBeenCalled()
    expect(tx.basketballTeam.count).not.toHaveBeenCalled()
  })
  ...
  it('checks existence before delete', async () => {
    ...
    expect(prisma.basketballMatchRound.delete).not.toHaveBeenCalled()
  })
})

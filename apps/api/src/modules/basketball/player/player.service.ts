...
export class PlayerService {
  ...
  create(dto: CreatePlayerDto) {
    return this.prisma.basketballPlayer.create({ data: dto })
  }
  ...
  async findAll(params: PlayerListParams = {}) {
    ...
    const [list, total] = await this.prisma.$transaction([
      this.prisma.basketballPlayer.findMany({
        where,
        skip: (page - 1) * pageSize,
        take: pageSize,
        orderBy: { playerNumber: 'asc' },
      }),
      this.prisma.basketballPlayer.count({ where }),
    ])
  ...
  async findOne(id: number) {
    const player = await this.prisma.basketballPlayer.findUnique({ where: { id } })
    if (!player) {
  ...
  async update(id: number, dto: UpdatePlayerDto) {
    ...
    return this.prisma.basketballPlayer.update({ where: { id }, data: dto })
  }

  async remove(id: number) {
    await this.findOne(id)
    return this.prisma.basketballPlayer.delete({ where: { id } })
  }
}
...
function buildPlayerWhere(params: PlayerListParams) {
  ...
  const where: Prisma.BasketballPlayerWhereInput = {}
